...
import { TeeType, TeeConfig, StateMetadata, StateVerification } from "./types";
...
export class VerifiableState {
    private static _instance: VerifiableState;
    ...
    private verifications: StateVerification[] = [];
    ...
    async initialize(runtime: IAgentRuntime): Promise<void> {
        ...
        this.teeConfig.enabled = enableValues.includes(
            enableTeeLog.toLowerCase()
        );
        ...
        if (useSgxGramine) {
            ...
            if (!this.teeConfig.simulation) {
                const { checkSGXEnvironment } = await import(
                    "./utils/sgxUtils"
                );
                if (!checkSGXEnvironment()) {
                    throw new Error(
                        `SGX environment not available.\n` +
            ...
            throw new Error(
                "Only SGX_GRAMINE is supported when TEE is enabled"
            );
        }
        ...
        if (this.teeConfig.enabled && !this.teeConfig.teeType) {
            throw new Error(
                "TEE is enabled but no valid TEE type is configured."
            );
        }
    ...
    async getState(stateName: string, key: string): Promise<StateMetadata> {
        ...
        try {
            ...
            throw new Error(
                `Error getting state ${stateName}.${key}: ${error}`
            );
        }
    }
    ...
    async verifyState(
        ...
        try {
            ...
            throw new Error(
                `Error verifying state ${stateName}.${key}: ${error}`
            );
        }
    }

    private async generateAttestation(data: any): Promise<string> {
        const dataStr = JSON.stringify(data);
        if (this.teeConfig.teeType === TeeType.SGX_GRAMINE) {
            if (this.teeConfig.simulation) {
                // 模拟模式下生成模拟认证
                const simulatedAttestation = {
                    report: Buffer.from(`simulated_report_${dataStr}`).toString(
                        "base64"
                    ),
                    signature: Buffer.from(
                        `simulated_signature_${Date.now()}`
                    ).toString("base64"),
    ...
    logVerification(result: StateVerification) {
...
export { verifiableStatePlugin } from "./services/verifiableStateService";
